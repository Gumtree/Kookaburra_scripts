...
# May 21 Need to properly take into account tube 6 or beam monitor
# 2021-05-21 Re-organise and integrate tube 6 OK
# 2021-05-21: Check that single point works properly - works in batch and stitch mode
...
g0 = Group('SAMPLE')
g0.numColumns = 3
g0.add(SampleName_FromFile,SampleName_Patching,SampleName,
...
g1 = Group('EMPTY SAMPLE CONTAINER')
g1.numColumns = 2
g1.add(empFiles, empFilesTakeBtn,empIgnorePts,  empLevel, 
...
g2 = Group('Optional: Determine Ambient Background')
g2.numColumns = 2
g2.add(bkgFiles, bkgFilesTakeBtn, bkgIgnorePts, 
...
use_tube6 = Par('bool', False)
use_tube6.title = 'Use Tube 6 as BM'

defaultMCR6 = Par('float', '2300')
defaultMCR6.title = 'Default MCR6'

medianMCR6 = Par('string', '0') 
medianMCR6.title = 'Median MCR6'
medianMCR6.enabled = False


use_beammonitor = Par('bool', False)
use_beammonitor.title = 'Use Beam Monitor'

defaultMCR = Par('float', '2300')
...
CreateBatchOverView = Par('bool', False)
...
g1 = Group('Advanced Settings - Beam Monitor and Tube 6')
g1.numColumns = 3
g1.add(use_tube6, defaultMCR6, medianMCR6, use_beammonitor, defaultMCR, medianMCR,use_medianMCRPlot)
...
def __run_script__(fns):
    ...
    buffer = list(ds.Tube6Ctr)
    buffer.sort()
    n = len(buffer)
    if n == 0:
        medianMCR6.value = 'nan'
    elif n % 2 == 1:
        medianMCR6.value = '%.0f' % (buffer[n/2])
    else:
        medianMCR6.value = '%.0f' % ((buffer[n/2 - 1] + buffer[n/2]) / 2.0)    
        ...
        with open(path + filename_out, 'w') as fp:
            ...
            for j in range(len(dsFilePaths)):
                ...
                for i in range(len(summary_all[j])):                
                    fp.write(filename +',') # note that this needs to be fixed
...
class ReductionDataset:
    ...
    def __init__(self, path):
        ...
        self.defaultMCR6 = defaultMCR6.value
        ...
        if use_tube6.value and use_beammonitor.value:
            print 'Please choose if the beam monitor ir tube 6 should be used to normaise flux'
                
        if use_tube6.value:        
           mcr = self.defaultMCR6
...
def PlotTube67Dataset(plot, ds, tube, title): # note that tube is here so that it can be replaced with other tube in future
    data = zeros(len(ds.Qvals))
    if tube == 'tube6':
        #data[:]     = ds.Tube6Ctr[:]
        if use_medianMCRPlot.value:            
            data[:]     = Array(ds.Tube6Ctr)/ds.defaultMCR6
        else:
            data[:]     = ds.Tube6Ctr[:]
        data.var[:] = 0
