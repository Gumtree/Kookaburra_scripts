...
defaultMCR_tube6 = Par('float', '2300')
...
def reduceBatchFiles():
    ...
    if CreateBatchOverView_Tsas.value:
        # add this for time dependent data 2019
        summary_all_Tsas = []# 14.1.2017 NEW
        file_start_times_Tsas = []
        file_names_Tsas = []
        #set earliest time to a point in the future so it will be immediately replaced
        earliest_time_Tsas = datetime.strptime('2100-01-01 00:00:00','%Y-%m-%d %H:%M:%S') #set this to some time in the future 
        header_Tsas = ["filename", "time [s]", "q [A^-1]", "Int [cm^-1]", "dI [cm^-1]"]

        if TemperatureVTE_setpoint.value:
            header_Tsas.append("VTE_SP [C]")
        if TemperatureVTE_setpoint.value:
            header_Tsas.append("VTE [C]")
        if TemperatureLSC.value:
            header_Tsas.append("LSC [C]")
        if TemperatureLSD.value:
            header_Tsas.append("LSD [C]")
    ...
    for dsFilePath in dsFilePaths:
        ...
        if CreateBatchOverView_Tsas.value: # 29.12.2021 for kinetic runs extract peakvalue etc
        
            summary_i_Tsas = [] #14.1.2017 Add to get an overview for files with few q-values
            ...
            if filetime < earliest_time :  #set earliest_time to the earliest file start_time
                earliest_time = filetime 
            file_start_times_Tsas.append(filetime)
            
            file_names_Tsas.append(filename)
            file_start_times_Tsas.append(filename)
            summary_i_Tsas.append(ds.PeakAng)
            summary_i_Tsas.append(ds.PeakVal)
            summary_i_Tsas.append(ds.TWideCtr)
            
            
            #print summary_i_Tsas
    ...
    if CreateBatchOverView_Tsas.value:
        ...
        print "ds.PeakAng", ds.PeakAng
        print "ds.PeakVal", ds.PeakVal
        print "ds.TransRock", ds.TransRock
        print "ds.TransWide", ds.TransWide
        print "em.PeakAng", em.PeakAng
        print "em.PeakVal", em.PeakVal
